...
class TestSend:
    ...
    @pytest.mark.asyncio
    async def test_typing_is_noop_and_final_reply_is_markdown(self):
        """Post-upgrade semantics: the WeCom busy bubble comes from the stream
        consumer seed frame (send_typing is a no-op), and send() delivers the
        final content as one markdown reply."""
        from plugins.platforms.wecom.adapter import WeComAdapter
        ...
        body = adapter._send_reply_request.await_args_list[0].args[1]
        assert body["msgtype"] == "markdown"
        assert body["markdown"]["content"] == "final answer"
...
class TestWeComZombieSessionFix:
    ...
    @pytest.mark.asyncio
    async def test_typing_after_final_reply_sends_nothing(self):
        """send_typing is a no-op on WeCom (bubble is stream-consumer owned),
        so no extra frames may be sent after delivery."""
        from plugins.platforms.wecom.adapter import WeComAdapter
