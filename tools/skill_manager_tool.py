...
def _resolve_skill_dir(name: str, category: str = None) -> Path:
    ...
    of the profile-local skills dir.  Falls back to the current Work user's
    private root (hermes-work) so a scoped WeCom session never writes into
    the shared tier.
    """
    base = None
    try:
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    # User-first ordering (hermes-work): a Work user's private skill shadows
    # a same-named public skill; the categorized relative form above still
    # resolves against the profile-local root only.
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "op only). Existing skills are modified wherever they live. Work "
        "users (scoped WeCom session) may modify only their private skills; "
        "administrator CLI sessions may maintain public skills. Keep "
